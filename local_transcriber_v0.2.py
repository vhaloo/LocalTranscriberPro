...
class StdErrRedirector:
    ...
    def write(self, buf):
        self.original_stderr.write(buf) # Keep original output
        # Parse tqdm percentage (e.g., " 45%|####")
...
class AudioRecorder:
    ...
    def start(self, device_index):
        logging.info(f"Starting recorder on device {device_index}")
...
class TranscriberApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        self.desc_label = ctk.CTkLabel(self.header_frame, text="Secure, local speech-to-text. Select your hardware and model below.", 
        ...
        # Microphone
        ctk.CTkLabel(self.settings_frame, text="Microphone:", font=("Roboto", 14)).pack(side="left", padx=(15, 5), pady=10)
        self.device_combo = ctk.CTkComboBox(self.settings_frame, width=300)
        self.device_combo.pack(side="left", padx=5, pady=10)
        self.populate_devices()

        # Model
        ctk.CTkLabel(self.settings_frame, text="Model:", font=("Roboto", 14)).pack(side="left", padx=(15, 5), pady=10)
        self.model_combo = ctk.CTkComboBox(self.settings_frame, values=list(MODEL_SIZES.values()), width=160)
        self.model_combo.set(MODEL_SIZES["small"])
        self.model_combo.pack(side="left", padx=5, pady=10)

        # Processing Unit
        ctk.CTkLabel(self.settings_frame, text="Device:", font=("Roboto", 14)).pack(side="left", padx=(15, 5), pady=10)
        self.proc_combo = ctk.CTkComboBox(self.settings_frame, values=["Auto", "GPU (CUDA)", "CPU"], width=110, command=self.on_device_change)
        self.proc_combo.set("Auto")
        self.proc_combo.pack(side="left", padx=5, pady=10)

        if self.cuda_missing:
            self.fix_cuda_btn = ctk.CTkButton(self.settings_frame, text="⚠️ Enable GPU", fg_color="#e67e22", hover_color="#d35400", 
                                          command=self.open_cuda_help, width=100)
        ...
        self.load_frame.grid_remove() # Hide initially
    ...
    def populate_devices(self):
        ...
        input_devices = []
        default_idx = sd.default.device[0] # Get system default input device index
    ...
    def start_recording(self):
        ...
        self.load_frame.grid() # Show progress
        self.progress_bar.set(0)
        self.loading_label.configure(text="Initializing...")
        
        threading.Thread(target=self.init_and_record, args=(device_index, model_name, proc_mode), daemon=True).start()

    def init_and_record(self, device_index, model_name, proc_mode):
        ...
        try:
            ...
            if self.model is None or self.model_name != model_name:
                ...
                # Check if model likely needs download (not exact, but good for UI hint)
                # Whisper handles this internally, but our redirector will catch it
                self.model = whisper.load_model(model_name, device=device_str)
                self.model_name = model_name
                self.log_message(f"[System] Model loaded successfully.")
    ...
    def stop_recording(self):
        self.recorder.stop()
        self.status_bar.configure(text="Stopping & Saving...")
    ...
    def save_transcript(self):
        if not self.full_transcript:
            self.log_message("[System] No text recorded to save.")
            return
        ...
        try:
            ...
            self.after(0, lambda: self.log_message(f"\n[System] ✔ Saved to Desktop:\n{filename}"))
        except Exception as e:
            self.after(0, lambda: self.log_message(f"[Error] Could not save file: {e}"))
