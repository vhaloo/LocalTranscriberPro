...
py -0 >nul 2>&1
if !errorlevel! equ 0 (
    :: Check if any 64-bit version is available via launcher
    for /f "tokens=*" %%v in ('py -c "import sys; print('64' if sys.maxsize > 2**32 else '32')"') do (
        if "%%v"=="64" (
            set "PY_CMD=py"
            set "PYTHON_FOUND=1"
        )
...
if !PYTHON_FOUND! equ 0 (
    echo [MISSING] 64-bit Python not found.
    echo          (32-bit Python is incompatible with AI libraries).
    echo [INFO] Auto-installing Python 3.12 (64-bit)...
    echo [MISSING] 64-bit Python. Installing... >> "%LOG_FILE%"
...
echo [OK] 64-bit Python found: !PY_CMD! >> "%LOG_FILE%"
...
:: Use explicit paths to venv tools to avoid subshell issues
...
:: Fix the path detection quoting issue
...
if !SIZE! LSS 100000000 (
    echo [ERROR] The built file is too small. Check log. >> "%LOG_FILE%"
    goto :ERROR
)
